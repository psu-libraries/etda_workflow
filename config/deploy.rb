...
namespace :sidekiq do
  desc 'prepare sidekiq for termination and terminate'
  task :stop do
    on roles(:worker) do
      execute "sudo /bin/systemctl stop sidekiq_pool_#{:partner}"
    end
  end
  desc 'restart sidekiq'
  task :restart do
    on roles(:worker) do
      execute "sudo /bin/systemctl start sidekiq_pool_#{:partner}"
    end
  end
