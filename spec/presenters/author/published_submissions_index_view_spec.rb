...
RSpec.describe Author::PublishedSubmissionsIndexView do
  describe 'author view for released submissions' do
    ...
    submission = FactoryBot.create(:submission, :released_for_publication, public_id: 'public-id123', author:)
    let(:view) { described_class.new(author) }

    it "#title_link returns a link to the author's published submission" do
      expect(view.title_link(submission)).to eq("<span class='sr-only'>link to your submission #{submission.cleaned_title} opens in a new tab</span> <a target = blank href = '#{EtdUrls.new.explore}/catalog/#{submission.public_id}' class='title'> #{submission.cleaned_title} </a>")
    end
