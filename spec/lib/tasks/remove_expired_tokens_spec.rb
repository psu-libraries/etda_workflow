...
RSpec.describe "Rake::Task['tokens:remove_expired']", type: :task do
  ...
  let!(:committee_member_token_2) { FactoryBot.create :committee_member_token, token_created_on: (Date.today - 800.days) }
  let!(:committee_member_token_3) { FactoryBot.create :committee_member_token, token_created_on: (Date.today - 700.days) }
  ...
  xit 'removes expired committee member tokens' do
    ...
    expect(CommitteeMemberToken.find(committee_member_token_3.id)).to be_present
