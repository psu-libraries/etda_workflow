RSpec.describe "Unrelease a submission", :honors, :js, type: :integration do
...
RSpec.describe 'Unrelease a submission with Solr error', :honors, :js do
  let!(:program) { FactoryBot.create(:program, name: "Any Program", is_active: true) }
  let!(:degree) { FactoryBot.create(:degree, name: "Thesis of Sisyphus", is_active: true) }
  let!(:role) { CommitteeRole.first.name }
...
RSpec.describe 'Unrelease a legacy submission without missing data', :honors, :js do
  let!(:program) { FactoryBot.create(:program, name: "Any Program", is_active: true) }
  let!(:degree) { FactoryBot.create(:degree, name: "Thesis of Sisyphus", is_active: true) }
  let!(:role) { CommitteeRole.first.name }
  let(:admin) { FactoryBot.create :admin }
