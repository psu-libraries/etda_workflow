  ...
  unless InboundLionPathRecord.active?
    ...
    describe "Cancel" do
      it "does not save the committee" do
        ...
        submission.required_committee_roles.count.times do |i|
          ...
          fill_in "submission_committee_members_attributes_#{i}_email", with: "buck#{i}@hotmail.com"
        end
        click_link('Cancel')
    ...
    describe "save and continue submission" do
      it "saves the committee" do
        ...
        expect(submission.committee_members.first.access_id).to eq('pbm1') unless current_partner.graduate?
        expect(submission.committee_members.first.access_id).to eq('pbm1') if current_partner.graduate?
    ...
    describe "Remove an optional committee member", js: true do
      before do
        ...
        submission.required_committee_roles.count.times do |i|
          submission.committee_members << FactoryBot.create(:committee_member, name: "Professor Buck Murphy#{i}", email: "buck#{i}@hotmail.com", is_required: true, committee_role_id: roles[i].id)
        end
