class Author::SubmissionView < SimpleDelegator
  ...
  def step_two_description
    if status_behavior.collecting_committee?
      ...
      return (step_two_name + "<a href='" + "/author/submissions/#{id}/committee_members" + "' class='medium'>[review <span class='sr-only'>committee for submission '#{title}' </span>]</a>").html_safe unless head_of_program_is_approving?
