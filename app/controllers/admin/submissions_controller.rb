class Admin::SubmissionsController < AdminController
  ...
  def update
    ...
    SubmissionStatusUpdaterService.new(@submission).update_status_from_committee if @submission.status == 'waiting for committee review' || @submission.status == 'waiting for head of program review'
