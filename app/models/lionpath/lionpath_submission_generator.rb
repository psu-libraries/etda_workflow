class Lionpath::LionpathSubmissionGenerator
  ...
  def create_submission
    ...
    seq = 1
    5.times.each do
      com_role = CommitteeRole.where('committee_roles.degree_type_id = ? AND is_program_head = false', degree_type.id).sample
  ...
  private
    ...
    def submission_attrs
      degree_name = degree_type.slug == 'dissertation' ? 'PHD' : 'MS'
      {
        ...
        program_id: Program.where("programs.is_active = true AND programs.name LIKE '%#{degree_name}%'")
                           .uniq.sample.id,
